...
def categoryList():
    ...
    cursor.execute("SELECT t1.username , t1.id , t2.title , t2.price , t2.thumbnail , t2.id FROM courses as t2 INNER JOIN users as t1 on t1.id = t2.creator_id WHERE t2.category ILIKE %s", ("%" + category + "%" ,))
    courses = cursor.fetchall()
...
def courseList():
    ...
    if exact_match:
        ...
        cursor.execute("SELECT t1.username , t1.id , t2.title , t2.price , t2.thumbnail , t2.id FROM courses as t2 INNER JOIN users as t1 on t1.id = t2.creator_id WHERE t2.category = %s OR t2.title ILIKE %s ORDER BY CASE WHEN t2.title ILIKE %s THEN 0 ELSE 1 END", (category, "%" + query + "%", "%" + query + "%"))
    else:
        cursor.execute("SELECT t1.username , t1.id , t2.title , t2.price , t2.thumbnail , t2.id FROM courses as t2 INNER JOIN users as t1 on t1.id = t2.creator_id WHERE t2.title ILIKE %s OR t2.description ILIKE %s OR t2.category ILIKE %s", ("%" + query + "%" , "%" + query + "%", "%" + query + "%"))
    courses = cursor.fetchall()
    cursor.execute("SELECT course_id FROM bookmarks where user_id = %s", (session["id"],))
    bookmarks = cursor.fetchall()
    conn.close()
...
def add_course():
    ...
    if not image or image.filename == "": return jsonify({"message": "No image selected"}), 400
    if not allowed_file(image.filename): return jsonify({"message": "Invalid image type"}), 400
...
def full_course():
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT t1.username , t2.title , t2.description , t2.category , t2.price , t2.thumbnail , t2.course_link,t2.creator_id FROM courses as t2 INNER JOIN users as t1 on t1.id = t2.creator_id WHERE t2.id = %s", (course_id,))
    course = cursor.fetchone()
    
    already_sub = False
    cursor.execute("SELECT t1.user_id , t1.creator_id FROM subscription as t1 INNER JOIN courses as t2 ON t1.creator_id = t2.creator_id WHERE t1.user_id = %s AND t2.id = %s", (session["id"] , course_id))
    ...
    cursor.execute("SELECT t1.username , t2.rating , t2.comment , t1.profile_pic FROM review as t2 INNER JOIN users as t1 on t1.id = t2.user_id WHERE t2.course_id = %s AND t1.id = %s", (course_id,session["id"]))
...
def creator_course():
    ...
    cursor.execute("SELECT t1.username , t1.id , t2.title , t2.price , t2.thumbnail , t2.id FROM courses as t2 INNER JOIN users as t1 on t1.id = t2.creator_id WHERE t2.creator_id = %s", (creator ,))
    courses = cursor.fetchall()
    cursor.execute("SELECT course_id FROM bookmarks where user_id = %s", (session["id"],))
    bookmarks = cursor.fetchall()
    conn.close()
